...
bool FatFSUSBClass::begin() {
    ...
    _sectSize = 512; //should be 512 in most cases (sd.bytesPerSector() is not exposed)
    _sectCount = sd.clusterCount() * sd.sectorsPerCluster();
    ...
    //read buffer
    _r_sectBuff = new uint8_t[_sectSize];
    _r_sectNum = -1;
...
extern "C" void tud_msc_capacity_cb(uint8_t lun, uint32_t* block_count, uint16_t* block_size) {
    (void) lun;
    *block_count = FatFSUSB.sectCount();
    *block_size  = FatFSUSB.sectSize();
}
...
int32_t FatFSUSBClass::read10(uint32_t lba, uint32_t offset, void* buffer, uint32_t bufsize) {
    ...
    if (_sectNum >= 0) {
        ...
        sd.card()->writeSector(_sectNum, _sectBuff);
        _sectNum = -1;
    }

    if(lba != _r_sectNum) {
        //read sector if not cached
        sd.card()->readSector(lba, _r_sectBuff);
        _r_sectNum = lba;
    }
    memcpy(buffer, _r_sectBuff + offset, bufsize);
    return bufsize;
}
...
int32_t FatFSUSBClass::write10(uint32_t lba, uint32_t offset, uint8_t* buffer, uint32_t bufsize) {
    ...
    if ((offset == 0) && (bufsize == _sectSize)) {
        sd.card()->writeSector(lba, buffer);
        return _sectSize;
    }
    ...
    if ((int)_sectNum == (int)lba) {
        ...
        sd.card()->readSector(lba, _sectBuff);
        memcpy(_sectBuff + offset, buffer, bufsize);
        _sectNum = lba;
    }
    ...
    if (offset + bufsize >= _sectSize) {
        ...
        sd.card()->writeSector(lba, _sectBuff);
        _sectNum = -1;
    }
    return bufsize;
}
...
void FatFSUSBClass::unplug() {
    if (_started) {
        ...
        //TODO fatfs::disk_ioctl(0, CTRL_SYNC, nullptr);
        if (_cbUnplug) {
