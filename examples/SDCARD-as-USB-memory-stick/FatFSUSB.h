...
public:
    ...
    uint16_t sectSize() {return _sectSize;}
    uint32_t sectCount() {return _sectCount;}
...
private:
    ...
    int32_t _sectNum = -1; //write buffer sector number, or -1 if no data needs to be writen
    uint8_t *_sectBuff = nullptr; //write buffer

    int32_t _r_sectNum = -1; //read buffer sector number, or -1 if no data is cached
    uint8_t *_r_sectBuff = nullptr; //read buffer

    uint16_t _sectSize = 0;
    uint32_t _sectCount = 0;
